    ...
    const GetBasketsNew = (ago) => {
        ...
        categories.map(category => {
            ...
            dates(ago).map(getDate => {
                ...
                totalCategoryDatePrice.push(totalPrice)
            })
            totalCategoryPrice.push(totalCategoryDatePrice)
        })

        console.log(totalCategoryPrice)
        ...
        const colors = ["`rgba(255,0,0,1)`", "`rgba(0,0,102, 1)`", `rgba(0,102,0,1)`]
        let portfoyCount = totalCategoryPrice.length
        console.log(portfoyCount)

        let dataNew = []

        if(portfoyCount > 0){
            for (let i = 0; i < portfoyCount; i++) {
                dataNew.push({
                    'data': totalCategoryPrice[i],
                    'strokeWidth': 2,
                    'color': () => colors[i]
                })
                console.log('sa')
